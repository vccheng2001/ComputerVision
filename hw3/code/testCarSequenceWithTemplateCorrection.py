...
for i in range(1, num_frames-1):
    ...
    rect_topleft = np.array([rect[0], rect[1]])

    # from 1 to i cumulative
    ...
    rect[0] += ptot[0] # add the correction 
    rect[1] += ptot[1]
    rect[2] += ptot[0]
    rect[3] += ptot[1]

    rect_wtc = rect.copy()
    rects_wtc.append(rect)
    ...
    if i == 1 or i % 100 == 0:
        ...
        ax.add_patch(box)
        plt.savefig(f'car-wtc-frame{i}.jpg')

    np.save('carseqrects-wcrt.npy',rects_wtc)