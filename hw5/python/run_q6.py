...
def main(args):
    ...
    if args.dataset == "nist":
        ...
        ********************************************************
        ...
        # classes 
        classes = np.array([_ for _ in string.ascii_uppercase[:26]] + [str(_) for _ in range(10)])
    else:
    ...
    for epoch in range(num_epoch):  
        ...
        for i, data in enumerate(train_loader):
            ...
            pred_label = classes[np.argmax(outputs.cpu().detach().numpy(), axis=1)]
            gt_label = classes[np.argmax(labels.cpu().detach().numpy(), axis=1)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="nist", help='dataset to use')
    parser.add_argument('--model', type=str, default="cnn", help='ML model')
